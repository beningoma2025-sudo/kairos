...
import { getFeaturedContent } from "@/lib/api/content";
...
export async function FeaturedHero() {
  ...
  return (
    <div className="relative h-[75vh] min-h-[500px] overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-end pb-20 px-8 max-w-[1800px] mx-auto">
        <div className="max-w-xl animate-fade-in">
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/watch/${hero.id}`}
              className="flex items-center gap-2 bg-white hover:bg-white/90 text-kairo-dark font-bold px-6 py-3 rounded-lg transition-all duration-200"
            >
            ...
            <Link
              href={`/watch/${hero.id}`}
              className="flex items-center gap-2 bg-white/20 hover:bg-white/30 text-white font-semibold px-6 py-3 rounded-lg backdrop-blur-sm transition-all duration-200"
            >
              <Info size={18} />
              More Info
            </Link>
      ...
      {/* Thumbnail strip for other featured content */}
      {featured.length > 1 && (
